...
syn match   messagesHour        contained display '\d[0-9]*:\d[0-9]*:\d[0-9]*.\d[0-9]* '
                                \ nextgroup=messagesPid

syn match   messagesPid        contained display '[a-z]*(\d[0-9]*) '
...
syn match   messagesText        contained display '.*'
                                \ contains=messagesProcess,messagesImportant
...
syn match   messagesProcess     contained '\(Sport ready\|Monitor service started\).*'
...
hi messagesProcess  term=bold ctermfg=LightBlue ctermbg=White guifg=#80a0ff  gui=bold
